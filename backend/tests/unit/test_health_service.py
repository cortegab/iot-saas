...
from app.health.service import _ON_CHANGE_MAX_AGE_SECONDS, derive_max_age
from app.rules.evaluators import DEFAULT_STALE_METRIC_AGE_SECONDS


def test_periodic_with_interval_uses_multiplier() -> None:
    assert derive_max_age("periodic", 30) == 90


def test_streaming_with_interval_uses_multiplier() -> None:
    assert derive_max_age("streaming", 5) == 15


def test_periodic_with_no_interval_falls_back_to_default() -> None:
    assert derive_max_age("periodic", None) == DEFAULT_STALE_METRIC_AGE_SECONDS
...
def test_on_change_ignores_interval_and_uses_generous_ceiling() -> None:
    ...
    """
    assert derive_max_age("on_change", 30) == _ON_CHANGE_MAX_AGE_SECONDS
    assert derive_max_age("on_change", None) == _ON_CHANGE_MAX_AGE_SECONDS
